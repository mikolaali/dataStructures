...
public class QuickSortStrategy2 implements SortStrategy{
    ...
    private void quickSort(int[] arr, int left, int right) {
        ...
        if (size < 4) {     // разбиение с медианой требует минимум 4х ячеек
            manualSort(arr, left, right); return;
        }
    ...
    private void manualSort(int[] arr, int left, int right) {
        int size = right - left + 1;
        if (size <= 1) return;
        if (size==2) {if(arr[left] > arr[right]) swap(arr, left, right);}
        else {
